...
async function smartFill(page, label, value, opts = {}){
  // 20 Sep 2026: never use Playwright's default 60s fill timeout.
  // Passport Number failed with "element is not visible" for 60s because a
  // hidden MUI input matched getByLabel while the real field was off-step
  // or not scrolled into view. This version: short timeouts, scroll into
  // view, try label / placeholder / name, then give up cleanly (no throw).
  if(!value) return { filled: false, reason: "no value" };
  const timeout = opts.timeout || 5000;
  const str = String(value);

  async function tryFill(el, method){
    try{
      await el.scrollIntoViewIfNeeded({ timeout: 2000 }).catch(() => {});
      const visible = await el.isVisible().catch(() => false);
      if(!visible){
        // MUI sometimes keeps a non-visible mirror input; try a short force fill once.
        await el.fill(str, { timeout: 2000, force: true });
        console.log(`[smartFill] "${label}": filled via ${method} (force)`);
        return true;
      }
      await el.fill(str, { timeout });
      console.log(`[smartFill] "${label}": filled via ${method}`);
      return true;
    }catch(e){
      console.warn(`[smartFill] "${label}": ${method} failed: ${e.message || e}`);
      return false;
    }
  }

  // 1) Every getByLabel candidate (visible first, then force)
  const candidates = await page.getByLabel(label, { exact: opts.exact || false }).all();
  for(const el of candidates){
    if(await tryFill(el, `getByLabel (${candidates.length} candidate(s))`)){
      return { filled: true, method: "getByLabel" };
    }
  }

  // 2) Label text → nearest following input
  try{
    const labelNode = page.getByText(label, { exact: false }).first();
    await labelNode.waitFor({ state: "attached", timeout: 3000 }).catch(() => {});
    const nearInput = labelNode.locator(
      "xpath=following::input[not(@type='hidden')][1] | following::textarea[1]"
    ).first();
    if(await nearInput.count()){
      if(await tryFill(nearInput, "text-proximity")){
        return { filled: true, method: "text-proximity" };
      }
    }
  }catch(e){ /* continue */ }

  // 3) Placeholder: match longest word from label (e.g. "Passport")
  try{
    const words = String(label).trim().split(/\s+/).filter(w => w.length > 2);
    const word = words.sort((a,b) => b.length - a.length)[0];
    if(word){
      const ph = page.locator(`input[placeholder*="${word}" i], textarea[placeholder*="${word}" i]`).first();
      if(await ph.count()){
        if(await tryFill(ph, "placeholder")){
          return { filled: true, method: "placeholder" };
        }
      }
    }
  }catch(e){ /* continue */ }

  // 4) input[name] rough match (passport_number, passportNumber, etc.)
  try{
    const key = label.toLowerCase().replace(/[^a-z0-9]+/g, "");
    const byName = page.locator(`input[name*="${key}" i], input[id*="${key}" i]`).first();
    if(await byName.count()){
      if(await tryFill(byName, "name/id heuristic")){
        return { filled: true, method: "name-id" };
      }
    }
  }catch(e){ /* continue */ }

  console.warn(`[smartFill] "${label}": no usable field (${candidates.length} getByLabel candidate(s)) — left blank, continuing wizard`);
...
async function fillAskUniApplication(page, app_row){
  ...
  await smartFill(page, "First Name", student.first_name || "");
  await smartFill(page, "Last Name", student.last_name || "");
  ...
  // Wait for step 02 (Student Information) to actually show before touching Passport.
  // Without this, getByLabel can match a hidden input from a previous step and hang.
  try{
    await page.getByText(/student information|passport number|birth date/i).first()
      .waitFor({ state: "visible", timeout: 15000 });
  }catch(e){
    console.warn("[fillAskUniApplication] step 02 heading/fields not visible yet — continuing best-effort");
  }
